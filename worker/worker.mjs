...
function agentPrompt(address, base) {
  return `Produce an NH ADU feasibility report, following AGENTS.md exactly. The target address is ${address} and the job basename is ${base}. First run: node tools/report.mjs "${address}" "${base}". This runs the engineering site-plan engine (tools/siteplan.cjs), which classifies every parcel grid cell as CLEARING (buildable open ground) vs OBSTRUCTION (tree canopy, roof/structure, pavement/deck, or pool/water), dissolves ALL the clearing inside the setbacks into ONE green ADU-eligible area polygon (with its total square footage — the whole usable region where an ADU could be sited), and places a sample 900 sf ADU box on the clearing nearest the house. Then VIEW the labeled grid image reports/${base}-grid.png with your image ability — a zoomed aerial with lettered/numbered cells, class tints (clearing=faint yellow, trees=green, structure/pavement=gray, pool/water=blue), the GREEN ADU-eligible area outline, and the current sample ADU box outlined in red. Going cell by cell with your OWN EYES, confirm (a) the green ADU-eligible polygon covers the genuinely open ground and excludes every pool, deck/patio, driveway, roof and tree, and (b) the red sample box sits fully on open CLEARING and is NOT on or touching a pool, pool deck/patio, driveway/pavement, house/roof, or trees. Trust your eyes over the tints. ⚠️ CRITICAL POOL-HOUSE TRAP: the pixel classifier CANNOT see a pool house / pool-equipment shed / pool deck — their flat tan roofs and tan/concrete aprons read as "clearing" (tan open ground), NOT as structure/gray or water/blue. A detected pool (blue cell) is almost ALWAYS ringed by this tan apron and a small outbuilding. So do NOT place the ADU on the tan ground immediately beside/around the pool even though it looks open — that is the pool house/deck. Look for the small building right next to the pool and the smoother tan apron around it, and keep the ADU a clear buffer (at least a couple of cells / ~30 ft) away from the WHOLE pool cluster, out on the larger open field/lawn further from the pool. If the box or the green area is wrong, pick the best OPEN-CLEARING cell that is well clear of the pool cluster (the open field east/away from the pool, NOT the tan apron hugging it) and re-run with MOLE_ADU_HINT="<that cell label>" node tools/report.mjs "${address}" "${base}", then view reports/${base}-grid.png again to confirm. Repeat until both the green area and the red box are unambiguously on open clearing clear of the pool CLUSTER (pool + deck + pool house), other structures and trees. YOU ARE THE REAL CLASSIFIER: if the pixel prior is clearly wrong you can override ANY cell by writing reports/${base}.classes.json (shape {"house":["B3"],"driveway":["A5"],"trees":["F9"],"clearable":["H10"],"lawn":["O8"],"pool":["L8"]} — unlisted cells keep the pixel prior) then re-running node tools/report.mjs. Do this especially when the classifier MISSED the house (tree-obscured roof — tag it so the ADU anchors right and never lands on the driveway) or scattered fake "open" cells across canopy gaps on a wooded lot (tag them as trees). TREES ARE CLEARABLE: if a full ADU does not fit on genuinely open ground but WOULD fit after removing some tree canopy, the engine returns aduScenario "with-clearing", paints the trees to remove as an ORANGE clear-to-build zone, and the verdict becomes "Feasible with tree clearing" (with sq ft + approx tree count). That is a valid, honest outcome for wooded lots — prefer it over pretending a canopy gap is lawn; a lot is only truly infeasible (aduScenario "none") if no full ADU fits even after clearing trees. When reporting tree clearing, flag protected/heritage trees, wetland buffers and any local tree-cutting ordinance to verify. Ensure reports/${base}.png, reports/${base}.md and reports/${base}.json all exist. Work autonomously; do not ask questions. End by printing DONE and the final cell.`;
}
