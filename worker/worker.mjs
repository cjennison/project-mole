...
function agentPrompt(address, base) {
  return `Produce an NH ADU feasibility report, following AGENTS.md exactly. The target address is ${address} and the job basename is ${base}. First run: node tools/report.mjs "${address}" "${base}". Then VIEW the labeled grid image reports/${base}-grid.png with your image ability — it is a zoomed aerial with lettered/numbered cells and the current ADU box outlined in red. Going cell by cell with your own eyes, identify which cells are open lawn vs a swimming POOL, pool deck/patio, house/roof, driveway, or trees. IMPORTANT: the pixel classifier cannot detect pools (a pool + decking looks tan, not blue) — trust your eyes, not the tints. If the red box is on or touching a pool, patio, driveway, roof, or trees, choose the best OPEN-LAWN cell nearest the house and re-run with MOLE_ADU_HINT="<that cell label>" node tools/report.mjs "${address}" "${base}", then view reports/${base}-grid.png again to confirm. Repeat until the ADU box is unambiguously on open ground clear of the pool and buildings. Ensure reports/${base}.png, reports/${base}.md and reports/${base}.json all exist. Work autonomously; do not ask questions. End by printing DONE and the final cell.`;
}
