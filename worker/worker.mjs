...
function agentPrompt(address, base) {
  return `Produce an NH ADU feasibility report, following AGENTS.md exactly. The target address is ${address} and the job basename is ${base}. Run node tools/report.mjs with that address and basename, then view reports/${base}.png with your image ability. If the red ADU box is on a pool, in trees, or on a building, correct it by re-running with MOLE_ADU_HINT set to a better open cell label, until the box is on open ground beside the existing house. Ensure reports/${base}.png, reports/${base}.md and reports/${base}.json all exist. Work autonomously; do not ask questions. End by printing DONE.`;
}
