...
export function buildReport(data, sitemap = {}) {
  ...
  const gates = [
    ...
    gate('Dimensional fit', lotOk === true ? 'pass' : lotOk === false ? 'warn' : 'unknown',
      ...
      + (sitemap.buildableAreaSqFt ? ` Buildable ≈ ${Number(sitemap.buildableAreaSqFt).toLocaleString()} sf; ADU-eligible open area ≈ ${Number(sitemap.effectiveAreaSqFt || 0).toLocaleString()} sf (the whole usable region — an ADU can be sited anywhere within it); sample 900 sf ADU ${sitemap.aduFitsSqFt ? 'fits' : 'needs review'}.` : ''),
