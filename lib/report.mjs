...
import { resolveAduMaxSqFt } from './towns.mjs';
...
export function buildReport(data, sitemap = {}) {
  ...
  // ADU siting scenario from the site-plan engine: 'as-is' (fits on open ground), 'with-clearing'
  // (only fits after removing some tree canopy), or 'none'. Drives the tree-clearing verdict tier.
  const needsClearing = sitemap.aduScenario === 'with-clearing';
  const clearingClause = needsClearing
    ? ` No ADU fits on existing open ground, but a ${sitemap.aduFitsSqFt || 900} sf ADU fits after clearing ≈ ${Number(sitemap.clearToBuildSqFt || 0).toLocaleString()} sf of tree canopy (~${sitemap.treesToClear || 0} tree${sitemap.treesToClear === 1 ? '' : 's'}); with-clearing buildable open area ≈ ${Number(sitemap.effectiveWithClearingSqFt || 0).toLocaleString()} sf.`
    : (sitemap.clearableTreeSqFt ? ` (≈ ${Number(sitemap.clearableTreeSqFt).toLocaleString()} sf of clearable tree canopy also lies within the envelope if more room is needed.)` : '');
  // ADU size: curated town override (if any) over the Atlas snapshot, clamped to the HB 577
  // statewide 750-950 sf band. Owner-occupancy: curated override wins over the Atlas value.
  const aduSize = resolveAduMaxSqFt({ atlasMax: z.aduMaxSqFt, override: tc.aduMaxSqFtOverride });
  const ownerOcc = tc.ownerOccupancyOverride ?? z.aduOwnerOccRequired;
  ...
  const gates = [
    gate('Legal / Zoning', sfAllowed ? 'pass' : (z.district ? 'warn' : 'unknown'),
      sfAllowed
        ? `${z.district}: single-family allowed → one ADU by right under RSA 674:72 (HB 577). Atlas ADU treatment "${z.aduTreatmentAtlas}" is a pre-2025 snapshot and is overridden by current state law. Enforceable ADU size by right: up to ${aduSize.effectiveMaxSqFt} sf${aduSize.clamped ? ` (local cap ${aduSize.localCap} sf adjusted to the statewide 750–950 sf band)` : ''}.`
        : `District ${z.district || 'unknown'}: confirm single-family / ADU treatment with the town.`,
      'NH Zoning Atlas + RSA 674:72'),
    gate('Dimensional fit', lotOk === true ? (needsClearing ? 'warn' : 'pass') : lotOk === false ? 'warn' : 'unknown',
      ...
      + (sitemap.buildableAreaSqFt ? ` Buildable ≈ ${Number(sitemap.buildableAreaSqFt).toLocaleString()} sf; ADU-eligible open area ≈ ${Number(sitemap.effectiveAreaSqFt || 0).toLocaleString()} sf (the whole usable region — an ADU can be sited anywhere within it); sample 900 sf ADU ${sitemap.aduFitsSqFt ? (needsClearing ? 'fits only with tree clearing' : 'fits') : 'needs review'}.` : '')
      + clearingClause,
  ...
  // Tree-clearing turns the dimensional gate into a warn. Subtract that single contribution ONLY
  // when the dimensional gate is warn PURELY because of clearing (lot size etc. are otherwise
  // fine). If the lot is also undersized (lotOk === false) that is a real, separate condition and
  // must NOT be hidden behind the tree-clearing tier.
  const dimOnlyClearing = needsClearing && lotOk !== false;
  const otherBlockers = blockers - (dimOnlyClearing ? 1 : 0);
  ...
  else if (needsClearing && otherBlockers === 0) { verdict = 'Feasible with tree clearing'; verdictClass = 'warn'; }
  else if (needsClearing) { verdict = 'Feasible with conditions + tree clearing'; verdictClass = 'warn'; }
  ...
  const snapshot = {
    ...
    aduMaxSqFt: aduSize.effectiveMaxSqFt, aduMaxSqFtSource: aduSize.source,
    aduMaxSqFtLocalCap: aduSize.localCap, aduMaxBedrooms: z.aduMaxBedrooms,
    ownerOccupancyRequired: ownerOcc,
    matchStrategy: p.matchStrategy, addressMatch: p.addressMatch, streetMatch: p.streetMatch,
  };
  ...
  if (p.streetMatch === false && p.streetWarning) dataGaps.push(p.streetWarning);
  ...
  // Local (town-by-town) notes: statewide clamp explanation + curated registry entries.
  const townNotes = [];
  if (needsClearing) townNotes.push(`🌳 **Tree clearing required:** a ${sitemap.aduFitsSqFt || 900} sf ADU does not fit on existing open ground, but fits after removing ≈ ${Number(sitemap.clearToBuildSqFt || 0).toLocaleString()} sf of tree canopy (~${sitemap.treesToClear || 0} tree${sitemap.treesToClear === 1 ? '' : 's'}, shown as the orange clear-to-build zone on the map). Confirm no protected/heritage trees, wetland buffers, or a local tree/clearing ordinance before removal.`);
  if (aduSize.note) townNotes.push(aduSize.note);
  if (tc.detachedAduNote) townNotes.push(tc.detachedAduNote);
  for (const n of (tc.notes || [])) townNotes.push(n);
  if (p.town && tc.curated === false) {
    townNotes.push(`No curated overrides on file for ${p.town} — using statewide defaults and the NH Zoning Atlas snapshot. Verify local ADU ordinance specifics with the town.`);
  }

  const report = { verdict, verdictClass, snapshot, gates, tasks, dataGaps, townNotes, generatedAt: new Date().toISOString() };
...
function toMarkdown(r, sitemap) {
  ...
  return [
    ...
    ``,
    ...(r.townNotes && r.townNotes.length ? [
      `## Local notes (${r.snapshot.town || 'town'})`,
      ...r.townNotes.map(n => `- ${n}`),
      ``,
    ] : []),
    `## Data gaps`,
